...
def crop_board(image: Image) -> Image:
    ...
    cv2.imshow('board_cropped', cropped_image)
    cv2.waitKey(1)
    input('')
...
def label_chessboard(board: Image) -> Image:
    ...
    cv2.imshow('board_labelled', board)
    cv2.waitKey(1)
    input('')
...
def remove_bg_with_green_contours(image: Image) -> Image:
    ...
    mask = cv2.inRange(hsv_image, green_low, green_high)
    ...
    cv2.imshow('board_contours', output)
    cv2.waitKey(1)
    input('')
...
def crop_moved_squares(image: Image) -> Image:
    ...
    if cropped_images:
        ...
        cv2.imshow('squares_cropped.png', np.hstack(resized_images))
        cv2.waitKey(1)
        input('')
...
def detect_move_made(prev_state: Image, curr_state: Image) -> list[str]:
    ...
    prev_state = crop_board(prev_state)
    curr_state = crop_board(curr_state)
    ...
    cv2.imshow('board_diff.png', curr_state)
    cv2.waitKey(1)
    input('')
